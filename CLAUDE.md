...
| 信源 | 性质 | 提供 |
|---|---|---|
| `~/.claude/sessions/<pid>.json` | Claude Code 的活会话注册表，**未公开实现**。看板只读它，唯一例外是残留 GC（`registryGc`，身份靠 `pid + procStart` 双验）| 有哪些会话 / 死活（pid + procStart）/ 忙闲（status） |
...
已废除「收起」「折叠」两个词。复选框叫「显示全部」（它管的不只手动隐藏那一档）。2026-08-13 加入第三个词「删除记录」，2026-08-18 加入第四个「结束进程」—— 四个是**四种不同的动作**，不是同义词：隐藏留数据可找回；删除真删记录且不可撤销（铁律 6b）；结束进程动的是**真实进程**而非记录，只对已脱钩的行出现（见 [`docs/detached-sessions.md`](docs/detached-sessions.md)）。选「删除」不选「清除」，因为后者听起来像批量操作。**扩到第五个词之前先证明它不是这四个的同义词** —— 当年封顶三个词是为了挡同义词泛滥，不是为了挡真的新动作。
...
- **强杀 / 直接关终端 / 关机不触发 `SessionEnd`**，记录会残留，这件事本身绕不过（transcript 文件锁、`~/.claude/ide/*.lock` 两条替代判活路子实测都不通）。但**后果已经能收干净了**：2026-08-18 起 console 探测能认出「进程还活着、终端没了」这一类（判据、procStart 精度陷阱、为何落看板不落 hook，全在 [`docs/detached-sessions.md`](docs/detached-sessions.md)），配合注册表 GC + 「删除记录」/ 自动删除（铁律 6b）形成完整回收链。别再往"让 `SessionEnd` 触发"那个方向试，要绕的是它的后果。
